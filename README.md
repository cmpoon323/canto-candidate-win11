# 粵語候選字工具（Windows 11 可攜版 v0.9.1 Offline）
...
## v0.9.1 Offline：NumPad 選字

v0.9.1 保持 Rime-Cantonese 本機詞庫和零網絡候選，並加入 NumPad `1`–`9`。候選窗顯示時，數字鍵盤與主鍵盤數字鍵都會選取目前頁面的相應候選；沒有候選時，NumPad 數字會交回原本程式正常輸入。NumPad Enter 沿用 Windows 的 `VK_RETURN`，會與主 Enter 一樣確認目前焦點。
...
每次交付都附帶 SHA-256。請在 Windows 開啟 PowerShell 並執行以下命令，再與 `CHECKSUMS-SHA256-v0.9.1.txt` 比較：

```powershell
Get-FileHash .\CantoCandidate-Win11-Portable-v0.9.1.zip -Algorithm SHA256
```
